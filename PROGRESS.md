...
> 최종 업데이트: 2026-05-06 (대본 URL 전체 분석 / 문장 단위 줄바꿈)
...
- [v] 대본 — URL 전체 분석 및 표시 개선
  ...
  - tab_script.py: 빈 [] 버그 수정 → 인트로/본문/아웃트로 한국어 라벨
  - tab_script.py: 문장 부호(. ! ?) 기준 한 줄씩 줄바꿈 표시
  - requirements.txt: trafilatura 추가
- [v] 대본 탭 쇼츠 특화 리뉴얼
  - 목표 길이 단위 분 → 초 (target_duration_sec), 기본값 50초
  - 길이 버튼: 30초/50초/1분30초/2분
  - 시스템 프롬프트: 아시아 정세 쇼츠 채널 (40~60대) 전용으로 재작성
  - 에디터 폰트 mono 제거, 다른 탭과 통일
  - 예상 시간 표시 초 단위로 변경
- [v] 소재 수집 — 타임아웃 오류 수정
  - api_client.py: collect_sources 타임아웃 30초 → 180초 (수집+번역 처리 시간 반영)
  - _post()에 timeout 파라미터 추가
